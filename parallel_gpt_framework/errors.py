...
class ParallelGPTError(Exception):
...
class ConfigurationError(ParallelGPTError):
...
class ProcessingError(ParallelGPTError):
...
class DecisionMakerError(ParallelGPTError):
...
class ValidationError(ParallelGPTError):
...
class APIError(ParallelGPTError):
...
class TimeoutError(ParallelGPTError):
...
class RateLimitError(ParallelGPTError):
...
class AuthenticationError(ParallelGPTError):
...
class ModelError(ParallelGPTError):
...
def get_error_info(error: ParallelGPTError) -> dict:
    """
